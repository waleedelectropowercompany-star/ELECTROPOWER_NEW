...
function ReportsPage({ role }) {
  ...
  async function exportInvoicesPDF(type) {
    const source =
      type === 'collected'
        ? invoices.filter(
            (x) => x.status === 'محصلة'
    ...
    if (!source.length) {
      alert(
        type === 'collected'
          ? 'لا توجد فواتير محصلة للتصدير'
          : 'لا توجد فواتير غير محصلة للتصدير'
      )
      return
    }
    ...
    const title =
      type === 'collected'
        ? 'تقرير الفواتير المحصلة'
        : 'تقرير الفواتير غير المحصلة'
      ...
      .map(
        (x) => `
          <tr>
            ...
            <td>${x.status || '-'}</td>
            <td>${x.collection_number || '-'}</td>
            <td>${x.collection_bank || '-'}</td>
          </tr>
        `
      )
      .join('')
    ...
    try {
      ...
      pdf.save(
        type === 'collected'
          ? 'الفواتير-المحصلة.pdf'
          : 'الفواتير-غير-المحصلة.pdf'
      )
    } finally {
 ...
 {role === 'admin' && (<ListCard`r`n title=تقرير المرتبات"
        loading={false}
          ...
          <tbody>
            {collections.map((x) => (
              <tr key={x.id}>
                ...
                <td style={styles.td}>
                  {Array.isArray(x.invoice_ids) ? x.invoice_ids.map(id => invoices.find(inv => inv.id === id)?.invoice_number || id).join('، ') : '-'}
                </td>
                <td style={styles.td}>
                  {x.statement || '-'}
                </td>
